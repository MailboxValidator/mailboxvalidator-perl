# This Makefile is for the MailboxValidator::SingleValidation extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/MailboxValidator/SingleValidation.pm]
#     AUTHOR => [q[MailboxValidator.com <support@mailboxvalidator.com>]]
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => {  }
#     DEFINE => q[]
#     INC => q[-I.]
...
FULLEXT = MailboxValidator\SingleValidation
BASEEXT = SingleValidation
PARENT_NAME = MailboxValidator
DLBASE = $(BASEEXT)
VERSION_FROM = lib/MailboxValidator/SingleValidation.pm
INC = -I.
DEFINE = 
OBJECT = 
LDFROM = $(OBJECT)
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/MailboxValidator/SingleValidation.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)\MailboxValidator
INST_ARCHLIBDIR  = $(INST_ARCHLIB)\MailboxValidator
...
TO_INST_PM = lib/MailboxValidator/SingleValidation.pm

PM_TO_BLIB = lib/MailboxValidator/SingleValidation.pm \
	blib\lib\MailboxValidator\SingleValidation.pm
...
SingleValidation.def: Makefile.PL
	$(PERLRUN) -MExtUtils::Mksymlists \
     -e "Mksymlists('NAME'=>\"MailboxValidator::SingleValidation\", 'DLBASE' => '$(BASEEXT)', 'DL_FUNCS' => {  }, 'FUNCLIST' => [], 'IMPORTS' => {  }, 'DL_VARS' => []);"
...
manifypods : pure_all  \
	lib/MailboxValidator/SingleValidation.pm
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/MailboxValidator/SingleValidation.pm $(INST_MAN3DIR)\MailboxValidator.SingleValidation.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) "abstract: \"Email verification module for Perl using MailboxValidator API. It validates if the email is valid, from a free provider, contains high-risk keywords, whether it's a catch-all address and so much more.\"" >> META_new.yml
	$(NOECHO) $(ECHO) author: >> META_new.yml
	$(NOECHO) $(ECHO) "  - 'MailboxValidator.com <support@mailboxvalidator.com>'" >> META_new.yml
	...
	$(NOECHO) $(ECHO) "name: MailboxValidator-SingleValidation" >> META_new.yml
	...
	$(NOECHO) $(ECHO) "   \"abstract\" : \"Email verification module for Perl using MailboxValidator API. It validates if the email is valid, from a free provider, contains high-risk keywords, whether it's a catch-all address and so much more.\"," >> META_new.json
	$(NOECHO) $(ECHO) "   \"author\" : [" >> META_new.json
	$(NOECHO) $(ECHO) "      \"MailboxValidator.com ^<support^@mailboxvalidator.com^>\"" >> META_new.json
	$(NOECHO) $(ECHO) "   ]," >> META_new.json
	...
	$(NOECHO) $(ECHO) "      \"lgpl_3_0\"" >> META_new.json
	$(NOECHO) $(ECHO) "   ]," >> META_new.json
	$(NOECHO) $(ECHO) "   \"meta-spec\" : {{" >> META_new.json
	$(NOECHO) $(ECHO) "      \"url\" : \"http://search.cpan.org/perldoc?CPAN::Meta::Spec\"," >> META_new.json
	$(NOECHO) $(ECHO) "      \"version\" : \"2\"" >> META_new.json
	$(NOECHO) $(ECHO) "   }}," >> META_new.json
	$(NOECHO) $(ECHO) "   \"name\" : \"MailboxValidator-SingleValidation\"," >> META_new.json
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e "pm_to_blib({{@ARGV}}, '$(INST_LIB)\auto', q[$(PM_FILTER)], '$(PERM_DIR)')" -- \
	  lib/MailboxValidator/SingleValidation.pm blib\lib\MailboxValidator\SingleValidation.pm 
